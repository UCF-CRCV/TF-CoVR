        ...
        if isinstance(items[0], torch.Tensor):
            ...
            if all(item.shape == items[0].shape for item in items):
                ...
                if items[0].dim() == 1:  # Handling 1D tensors (e.g., sequences)
                    ...
                    max_shape = (
                        torch.tensor([list(item.shape) for item in items])
                        .max(dim=0)[0]
                        .tolist()
                    )
                    ...
                    for item in items:
                        ...
                        for item_dim, max_dim in zip(item.shape[::-1], max_shape[::-1]):
                            ...
                            if pad_value < 0:
                                raise ValueError(
                                    f"Negative padding encountered: {pad_value}"
                                )
                            padding.extend(
                                [0, pad_value]
                            )  # (left_pad, right_pad) format
...
class WebVidCoVRDataset(Dataset):
    def __init__(
        ...
        if si_tc_weight > 0:
            ...
            assert (
                txt2emb_pth.exists()
            ), f"txt2emb does not exist: {txt2emb_pth}. Please compute them with: python tools/embs/save_{model}_embs_txts.py {self.annotation_pth} {self.emb_dir}"
            txt2emb_pth = self.emb_dir / f"txt2_{self.annotation_pth.stem}.pth"
