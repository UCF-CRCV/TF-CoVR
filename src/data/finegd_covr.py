...
class FineGDCoVRDataset(Dataset):
    def __init__(
        ...
        self.df["modification"] = self.df["modification"].apply(eval)
    ...
    def __getitem__(self, index):
        ...
        source_label = ann["label1"]
        target_label = ann["label2"]

        if self.model == "clip":
            reference_vid = torch.load(reference_pth, map_location="cpu").to(
                torch.float32
            )
        ...
        modification = ann["modification"][str(target_label)]["modification"]
        ...
        if self.model == "clip":
            target_emb = (
                torch.load(target_pth, map_location="cpu").cpu().to(torch.float32)
            )
